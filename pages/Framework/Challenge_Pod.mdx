...
graph TD
    ...
    subgraph "Challenge Resources"
        Secret["Flag Secret"]
        ConfigMap["Challenge ConfigMap"]
    end

    User -->|"HTTPS"| Ingress
    Ingress -->|"WebOS URL"| WebOS
    Ingress -->|"Terminal URL"| RT
    RT -->|"Kubernetes API"| ChallengeOS
    WebOS -->|"Apps Config"| RT
    Secret --> WebOS
    ConfigMap --> ChallengeOS
```
...
sequenceDiagram
    ...
    User->>Dashboard: Start Challenge
    Dashboard->>IM: POST /api/start-challenge with CDF
    IM->>IM: Process CDF & load CTD
    IM->>IM: Generate flags & variables
    IM->>K8s: Create Pod, Service, Ingress
    K8s->>Pod: Deploy Challenge Pod
    IM->>Dashboard: Return URLs
    Dashboard->>User: Provide Challenge Links
    User->>Pod: Access via WebOS & Terminal
```
