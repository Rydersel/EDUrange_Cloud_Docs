...
metadata:
  ...
  labels:
    app: challenge
spec:
  serviceAccountName: terminal-account  # Service account for terminal access
  containers:
  - name: challenge-container
    image: registry.rydersel.cloud/debiantest  # Gets overwritten dynamically
    ports:
    - containerPort: 22
      name: ssh
    securityContext:
    ...
    env:
    - name: FLAG
      value: "flag"  # Gets overwritten with a unique flag
  - name: bridge
  ...
  - name: terminal
    ...
    - name: CONTAINER_NAME
      value: "challenge-container"
    - name: POD_NAME
      value: "instance-name"  # Gets overwritten with actual pod name
    - name: KUBERNETES_HOST
      value: "kubernetes-api-server"  # Gets overwritten with actual K8s host
    - name: KUBERNETES_NAMESPACE
      value: "default"
    - name: KUBERNETES_SERVICE_ACCOUNT_TOKEN
      value: "service-account-token"  # Gets overwritten with actual token
```

**2. Service**

* Exposes the application running in the pod to other services within the cluster.
...
spec:
  ...
  - protocol: TCP
    port: 3001
    targetPort: 3001  # Expose the Terminal container
    name: terminal
  type: ClusterIP
```
      ...
      - path: /
        ...
        backend:
          service:
            ...
            port:
              number: 80
  - host: terminal-challenge.rydersel.cloud  # This will be replaced dynamically
    http:
      paths:
      - path: /
        pathType: ImplementationSpecific
...
The Instance Manager now includes a Remote Terminal container in each challenge pod, which provides a more direct and secure way for users to interact with the Challenge OS container. This replaces the previous approach that relied more heavily on the Bridge component.

**Key Configuration for Remote Terminal:**

1. **Service Account Setup:**
   - The pod uses a dedicated service account (`terminal-account`) with specific permissions to interact with the Kubernetes API.

2. **Environment Variables:**
   - The Instance Manager injects several critical environment variables into the Terminal container:
     ```yaml
     env:
     - name: CONTAINER_NAME
       value: "challenge-container"  # Target container to connect to
     - name: POD_NAME
       value: "instance-name"  # Actual pod name
     - name: KUBERNETES_HOST
       value: "kubernetes-api-server"  # K8s API server address
     - name: KUBERNETES_NAMESPACE
       value: "default"  # Namespace where the pod runs
     - name: KUBERNETES_SERVICE_ACCOUNT_TOKEN
       value: "service-account-token"  # Token for API authentication
     ```

3. **WebOS Integration:**
   - The WebOS container is configured with the Terminal URL:
     ```yaml
     - name: NEXT_PUBLIC_TERMINAL_URL
       value: "https://terminal-$(POD_NAME).$(INGRESS_URL)"
     ```
...
4. **Ingress Configuration:**
   - A separate ingress rule is created for the Terminal service, allowing direct access via a dedicated URL.

This integration provides a more robust and secure terminal experience, with direct access to the Challenge OS container through the Kubernetes API rather than relying on the Bridge component as an intermediary.
